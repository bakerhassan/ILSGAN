...
def setup_training_loop_kwargs(
    ...
    args.training_set_kwargs = dnnlib.EasyDict(class_name='training.dataset.DRCDataset', data_name=data, path=real_data_abs_path, size=64,
    ...
    cfg_specs = {
        ...
        'ILS_L1': dict(
            ...
            ILS_form_config=dict(
                mode='anti_leakage',
                estimator='l1_distance',  # l1_distance, l1_distance_allowgrad, club
                distribution='predetermined_laplace',  # neural_gaussian, pixelcnnpp, predetermined_laplace, predetermined_gaussian
                scale='do_mean',
                neg_weight=1.0,
                # Only used in CLUB (ILS-MI)
        ...
        'ILS_predN': dict(
            ...
            ILS_form_config=dict(
                mode='anti_leakage',
                estimator='club',  # l1_distance, l1_distance_allowgrad, club
                distribution='predetermined_gaussian',  # neural_gaussian, pixelcnnpp, predetermined_laplace, predetermined_gaussian
                scale='do_mean',
                neg_weight=1.0,
                # Only used in CLUB
                mf_conditioned_b=dict(  # min MI(s*x|y)
        ...
        'ILS_learnL': dict(
            ...
            ILS_weight_strategy=dict(
                strategy='all_time',
                initial_anti=True,
                initial_anti_sigma=1e-3,  # 0.2
                ILS_sigma=1e-3,  # 5e-5
            ),
            ILS_form_config=dict(
                mode='anti_leakage',
                estimator='club',  # l1_distance, l1_distance_allowgrad, club
                distribution='neural_laplace',  # neural_gaussian, neural_laplace, pixelcnnpp, predetermined_laplace, predetermined_gaussian
                scale='do_mean',
                neg_weight=1.0,
                # Only used in CLUB
                mf_conditioned_b=dict(  # min MI(s*x|y)
        ...
        'ILS_learnN': dict(
            ...
            ILS_form_config=dict(
                mode='anti_leakage',
                estimator='club',  # l1_distance, l1_distance_allowgrad, club
                distribution='neural_gaussian',  # neural_gaussian, pixelcnnpp, predetermined_laplace
                scale='do_mean',
                neg_weight=1.0,
                # Only used in CLUB
                mf_conditioned_b=dict(  # min MI(s*x|y)
...
def main(ctx, outdir, dry_run, **config_kwargs):
    ...
    # Automatically do generation and segmentation
    print("\n=====> Start auto test <=====\n")
    if args.loss_kwargs.do_anti_leaking:
        synthetic_path = auto_test(
            path=args.run_dir,
            real_data=args.training_set_kwargs.data_name,
            scale=args.training_set_kwargs.size,
            sigma=None,
            aug='color',
            real_data_abs_path=real_data_abs_path,
        )
    else:
        synthetic_path = auto_test(
            path=args.run_dir,
            real_data=args.training_set_kwargs.data_name,
            scale=args.training_set_kwargs.size,
            # sigma=args.loss_kwargs.ILS_weight_strategy['ILS_sigma'],
            aug='color',
            real_data_abs_path=real_data_abs_path,
        )

    # Automatically do generation and segmentation
    print("\n=====> Start auto MI Estimation <=====\n")
    eval_MI(
